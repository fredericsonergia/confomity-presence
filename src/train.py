...
parser.add_argument(
    ...
    default=5,
    dest="save_interval",
    help="interval of epoch for saving models",
)
...
def train(net, ctx, train_data, val_data, val_data2, classes=["cheminee", "eaf"]):
    ...
    trainer = gluon.Trainer(
        net.collect_params(), "adam", {"learning_rate": 0.001, "wd": 0.0005}
    )
    ...
    for epoch in range(int(args.start_epoch), int(args.epoch)):
        ...
        for i, batch in enumerate(train_data):
            ...
            trainer.step(1)
            ce_metric.update(0, [loss * batch_size for loss in cls_loss])
            smoothl1_metric.update(0, [loss * batch_size for loss in box_loss])
...
if __name__ == "__main__":
    ...
    print(
        f"taille données train : {len(train_dataset)}",
        "taille données val: {len(val_dataset)}",
    )
    net = get_pretrained_model(classes)
    train_data, val_data, val_data2 = (
        train_dataloader(net, train_dataset),
        val_dataloader(val_dataset),
        train_dataloader(net, val_dataset),
    )
    ctx = get_ctx()
    ...
    (
        ...
        smooth_loss_val,
        map_list,
    ) = train(net, ctx, train_data, val_data, val_data2, classes)
    plot_train(
        epochs, ce_loss_list, ce_loss_val, smooth_loss_list, smooth_loss_val, map_list
    )
