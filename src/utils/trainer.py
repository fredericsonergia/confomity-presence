...
def ssd_val_dataloader(val_dataset, data_shape=512, batch_size=10, num_workers=0):
    ...
    width, height = data_shape, data_shape
    batchify_fn = Tuple(Stack(), Pad(pad_val=-1)) # stack image, cls_targets, box_targets
    val_loader = gluon.data.DataLoader(
        val_dataset.transform(SSDDefaultValTransform(width, height)), batch_size, shuffle=False,
