...
class Predictor(object):
    ...
    def train_from_pretrained(self, data_path='../Data/EAF_real', save_prefix= 'ssd_512_test2', start_epoch=0, epoch=10, save_interval=20, save_plot=True):
        detector = ModelBasedDetector.from_pretrained(data_path=data_path, base_model='ssd_512_mobilenet1.0_custom', save_prefix=save_prefix)
    ...
    def train_from_finetuned(self, save_prefix, data_path='../Data/EAF_real', model_name='models/ssd_512_best.params', start_epoch=0, epoch=10, save_interval=20, save_plot=True):
        detector = ModelBasedDetector.from_finetuned(model_name, data_path, save_prefix=save_prefix)
        epochs, ce_loss_list, ce_loss_val, smooth_loss_list, smooth_loss_val, map_list = detector.train(start_epoch, epoch, save_interval)
        plot_train(epochs, ce_loss_list, ce_loss_val, smooth_loss_list, smooth_loss_val, map_list, save_prefix, save_plot)
